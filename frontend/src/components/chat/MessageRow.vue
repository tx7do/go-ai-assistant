<template>
  <div class="flex flex-row lt-md:flex-col py-3 lt-md:py-2 px-4 relative" :style="{ backgroundColor: backgroundColor }">
    <div class="w-10 lt-md:ml-0 ml-2 mt-3">
      <n-avatar v-if="props.message.role == RoleEnum.USER" size="small">
        <n-icon>
          <PersonFilled/>
        </n-icon>
      </n-avatar>
      <ChatGPTAvatar v-show="!shrinkMessage" v-else size="small"/>
    </div>
...
import ChatGPTAvatar from '../avatar/ChatGPTAvatar.vue';
...
const renderPureText = computed(() => {
  return !appStore.preference.renderUserMessageInMd && props.message.role == RoleEnum.USER;
});
...
const backgroundColor = computed(() => {
  if (props.message.role == RoleEnum.USER) {
...
const copyMessageContent = () => {
  ...
  clipboard
      ...
      .then(() => {
        getWindow('$message').success(t('commons.copiedToClipboard'));
      })
      .catch(() => {
